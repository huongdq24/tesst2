...
const StartVideoGenerationInputSchema = z.object({
  ...
  referenceVideoUri: z.string().optional().describe(
    "Optional reference video snippet to extend via Video-to-Video continuation."
  ),
...
/**
 * Downloads a video from a URL and uploads it to the Gemini File API.
 * Returns the Gemini File URI (e.g., https://generativelanguage.googleapis.com/v1beta/files/abc123def)
 * Video payloads can be large (3-15MB) so we use a longer timeout.
 */
export async function uploadVideoToGemini(url: string, apiKey: string, timeoutMs: number = 60000): Promise<string | null> {
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), timeoutMs);

    console.log(`[VideoUpload] Downloading video from: ${url.substring(0, 50)}...`);
    const response = await fetch(url, { signal: controller.signal });

    if (!response.ok) {
      console.warn(`[VideoUpload] Failed to fetch video: ${response.status}`);
      clearTimeout(timeoutId);
      return null;
    }
    ...
    console.log(`[VideoUpload] Successfully uploaded video to Gemini. URI: ${uploadData.file.uri}`);
    return uploadData.file.uri;
  } catch (err: any) {
    if (err.name === 'AbortError') {
      console.error(`[VideoUpload] Timeout uploading video: ${url}`);
    } else {
      console.error(`[VideoUpload] Error uploading video: ${err.message}`);
    }
    return null;
  }
}
...
const startVideoGenerationFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    let referenceVideoPayload: any = null;
